<template>
  <div class="relative grid items-end justify-end w-screen h-screen bg-black"> 
    <!-- Canvas -->
    <canvas class="fixed inset-0"></canvas>
    
    <!-- Background -->
    <div class="absolute opacity-[0.075] inset-0 pointer-events-none">
      <div class="w-full h-full bg-gradient-to-r from-rose-500 to-violet-500 blur-3xl"></div>
    </div>

    <!-- Bar -->
    <div class="fixed inset-x-0 flex p-6 backdrop-blur">
      <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-white/5 via-white/25 to-white/5"></div>
      <!-- Button -->
      <div class="relative group">
        <div class="rounded-md relative z-10 cursor-pointer text-sm px-2 items-center py-1.5 flex text-gray-200">
          <svg width="100" height="100" viewBox="0 0 100 100" class="w-8 h-8" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="absolute transition duration-500 opacity-0 inset-2 from-violet-500 to-rose-500 group-hover:opacity-100 bg-gradient-to-r blur-md"></div>
      </div>
    </div>
